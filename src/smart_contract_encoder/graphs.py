...
def clones_eval_graphs():
    ...
    for json_file in glob.glob(os.path.join(results_dir, "*_clones_results.json")):
        model_name = os.path.basename(json_file).replace("_clones_results.json", "")
        model_name = model_renaming.get(model_name)
        if model_name is None:
            continue
        with open(json_file, 'r') as file:
    ...
    for metric in metrics:
        ...
        plt.xlabel("k")
        plt.ylabel(f"{metric.capitalize()}@k (Clones)")
        plt.xticks(k_values, rotation=45)
        plt.ylim(bottom=0)
        plt.grid(True, linestyle='-', linewidth=0.5, alpha=0.7)
        plt.legend()
